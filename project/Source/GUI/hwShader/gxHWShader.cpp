...
bool gxHWShader::loadShader(const std::string& shaderFile)
{
	...
	//read shader code
	int fileSz=0;
	FILE* fp=fopen(orig_filename.c_str(), "r");
	if(fp==NULL)
	{
		DEBUG_PRINT("Shader %s FILE open ERROR #1", orig_filename.c_str());
		return false;
	}
	...
	if(!fileSz)
	{
		DEBUG_PRINT("Shader %s FILE size ZERO ERROR", orig_filename.c_str());
		return false;
	}
	...
	fp=fopen(orig_filename.c_str(), "r");
	if(fp==NULL)
	{
		DEBUG_PRINT("Shader %s FILE open ERROR #2", orig_filename.c_str());
		return false;
	}
	...
	if(fp==NULL)
	{
		DEBUG_PRINT("Shader %s FILE open ERROR #3", orig_filename.c_str());
		return false;
	}
	...
	if (!compileShader(&vertexShaderID, GL_VERTEX_SHADER, vbuffer))
	{
		DEBUG_PRINT("Failed to compile vertex shader '%s'", orig_filename.c_str());
		GX_DELETE_ARY(vsource);
		GX_DELETE_ARY(fsource);
		return false;
	}
	...
	if (!compileShader(&fragmentShaderID, GL_FRAGMENT_SHADER, fbuffer))
	{
		DEBUG_PRINT("Failed to compile fragment shader '%s'", orig_filename.c_str());
		GX_DELETE_ARY(vsource);
		GX_DELETE_ARY(fsource);
		return false;
	}
	...
	if (!linkProgram())
	{
		DEBUG_PRINT("Failed to link program: %d (shader : %s)", shaderProgram, orig_filename.c_str());
		destroyShader();
		GX_DELETE_ARY(vsource);
		GX_DELETE_ARY(fsource);
		return false;
	...
	shaderName.assign(orig_filename);

	DEBUG_PRINT("Shader Loaded %s", orig_filename.c_str());
